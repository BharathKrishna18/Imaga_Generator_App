...
class ImageViewModel(config : Configuration) : ViewModel() {
    private val apikey = "Bearer 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"
    ...
    fun generateImage(prompt : String, callback: () -> Unit) {
        viewModelScope.launch {
            try {
                ...
                viewModelScope.launch {
                    val call = RetrofitInstance.openApi.generateImage(apiKey = apikey, requestBody = imageReq, contentType = "application/json")
